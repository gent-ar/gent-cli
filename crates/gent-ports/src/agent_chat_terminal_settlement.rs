//! Read-only restart snapshot for a durably settled provider-bound turn.

use gent_types::AgentChatTerminalSettlement;

use crate::LedgerError;

/// Reads a terminal snapshot only when dispatch settlement and turn terminality agree.
///
...
pub trait AgentChatTerminalSettlementReader: Send + Sync {
    /// Reads the latest durable terminal snapshot for one exact run and turn.
    ///
    /// # Errors
