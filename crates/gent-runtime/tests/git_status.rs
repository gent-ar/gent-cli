...
use gent_runtime::{GitStatusRequest, GitStatusService, GitStatusState};
...
fn observer_mode_never_claims_a_receipt_lease_or_git_effect() {
    ...
    let result = GitStatusService::new(ledger.clone(), fake.clone(), false)
...
fn authority_claims_receipt_and_lease_then_returns_a_durable_idempotent_summary() {
    let ledger = ledger_with_run();
    let fake = FakeGit::default();
    let service = GitStatusService::new(ledger.clone(), fake.clone(), true);
...
fn a_contended_worktree_rejects_before_git_execution() {
    ...
    ledger
        ...
        .unwrap();
    let fake = FakeGit::default();
    let result = GitStatusService::new(ledger, fake.clone(), true)
