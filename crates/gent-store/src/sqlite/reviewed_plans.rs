...
use super::reviewed_plan_values::{child_id, context_name, decode, invalid};
...
impl ReviewedPlanLedger for SqliteLedger {
    ...
    fn current_conversation_plan(
        ...
        pursuit::current(&*self.lock()?, conversation_id)
    }

    fn plan_turns_awaiting_review(&self) -> Result<Vec<gent_types::PlanTurn>, LedgerError> {
        pursuit::turns_awaiting_review(&*self.lock()?)
    }

    fn implementations_awaiting_prompt(
        &self,
    ) -> Result<Vec<gent_types::PlanImplementation>, LedgerError> {
        pursuit::implementations_awaiting_prompt(&*self.lock()?)
    }
}
...
fn save(ledger: &SqliteLedger, plan: &PlanArtifact) -> Result<(), LedgerError> {
    ...
    let reviewed_turn = tx
        .query_row(
            "SELECT content_digest_sha256 FROM reviewed_plan_artifacts WHERE source_turn_id = ?1",
            [&plan.source_turn_id],
            |row| row.get::<_, String>(0),
        )
        .optional()
        .map_err(storage_error)?;
    if let Some(digest) = reviewed_turn {
        return (digest == plan.content_digest_sha256)
            .then_some(())
            .ok_or_else(|| invalid("plan source turn conflict"));
    }
    ...
    pursuit::publish(
        &tx,
        &pursuit::stored(&tx, &plan.plan_id.0, plan.revision.0)?,
    )?;
...
fn approve(
    ...
    pursuit::publish(
        &tx,
        &pursuit::stored(&tx, &request.plan_id.0, request.plan_revision.0)?,
    )?;
    let receipt = Receipt {
...
fn reject(
    ...
    pursuit::publish(&tx, &pursuit::stored(&tx, &id.0, revision.0)?)?;
    tx.commit().map_err(storage_error)
}
...
fn current_parent(
    ...
    let active = tx
        .query_row(
            "SELECT 1 FROM turns WHERE run_id = ?1 AND phase IN ('active', 'waitingPermission', 'waitingQuestion') LIMIT 1",
            [&request.parent_run_id.0],
            |_| Ok(()),
        )
        .optional()
        .map_err(storage_error)?;
    active.is_none().then_some(()).ok_or(LedgerError::Rejected(
...
#[path = "reviewed_plan_approval.rs"]
mod approval;
use approval::{command, history_ordinal, insert_child, result};
