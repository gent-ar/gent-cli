...
use super::{ClaudePromptDispatchOutcome, ClaudePromptExecution, ClaudePromptLifecycle};
    ...
    R: PublicProviderResolver,
{
    pub(super) fn release_outdated_session(&mut self, run_id: &str) -> Result<(), RuntimeError> {
        let idle = self
            .active
            .get(run_id)
            .is_some_and(|binding| binding.settled && binding.steers.is_empty());
        if idle
    ...
    pub(super) fn fail_run(
        ...
        eprintln!("Claude run {run_id} failed and was stopped: {error}");
        if self.runner.has_claude_session(run_id) {
            let _ = self.runner.release_claude_session(run_id);
        }
        if self
            .active
            .get(run_id)
    ...
    pub(super) fn fail_dispatch(
        ...
        eprintln!(
            "Claude prompt {} failed before its turn started: {error}",
        ...
        Ok(ClaudePromptDispatchOutcome::Unprovable {
