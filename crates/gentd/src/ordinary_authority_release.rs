...
impl VerifiedOrdinaryAuthorityRelease {
    ...
    pub(crate) fn authorizes_provider(&self, provider: &str) -> bool {
        self.providers.iter().any(|authority| {
            matches!(
                (authority, provider),
...
#[path = "ordinary_authority_release_support.rs"]
mod support;
use support::{canonical_payload, keys, read, valid_hex};

#[cfg(test)]
#[path = "ordinary_authority_release_fixture.rs"]
