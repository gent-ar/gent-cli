...
fn one_signed_release_binds_its_nested_authority_to_the_locked_node() {
    ...
    let verified =
        SignedOrdinaryAuthorityRelease::load_bound(&path, &root_keys(&signer), &runtime, 10)
            .unwrap();
    assert!(verified.authorizes_provider("codex"));
    assert!(!verified.authorizes_provider("claude"));
    assert_eq!(
    ...
    assert_eq!(
        ...
        format!(
            "0.1.0-{}",
            gent_adapters::provider_platform::host_platform()
                .unwrap()
                .npm
        )
    );
...
#[test]
fn unknown_data_and_changed_node_fail_before_authority_is_returned() {
    let root = tempfile::tempdir().unwrap();
    let runtime = runtime(root.path());
...
#[test]
fn authority_digest_uses_canonical_content_not_whitespace() {
    let root = tempfile::tempdir().unwrap();
    let runtime = runtime(root.path());
