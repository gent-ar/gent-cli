...
pub(crate) use mapping::activity_for_lifecycle;

pub(crate) fn recorded_wire_fact(
    noticed: &mut std::collections::BTreeSet<String>,
    turn_id: &str,
    fact: &PublicWireFact,
) -> Option<PublicWireFact> {
    use gent_drivers::public_protocol::PublicCompactionObservation as Observation;
    let classification = match fact {
        PublicWireFact::Compaction(Observation::Started) => return None,
        PublicWireFact::Compaction(Observation::Completed) => {
            gent_types::PROVIDER_CONTEXT_COMPACTED_DIAGNOSTIC
        }
        PublicWireFact::Compaction(Observation::Failed { .. }) => {
            gent_types::PROVIDER_CONTEXT_COMPACTION_FAILED_DIAGNOSTIC
        }
        fact => return Some(fact.clone()),
    };
    noticed
        .insert(format!("{turn_id}\u{0}{classification}"))
        .then(|| {
...
use mapping::{activity, terminal, transcript_content};
...
fn batch(
    ...
    let transcript = transcript_content(&input.fact).map(|(kind, text, is_partial)| {
        NormalizedTranscriptAppend {
            ...
            turn_id: input.turn_id.clone(),
            kind,
            text: gent_types::bounded_text(&text, gent_types::MAX_TRANSCRIPT_TEXT_BYTES)
                .into_owned(),
            is_partial,
        }
    });
