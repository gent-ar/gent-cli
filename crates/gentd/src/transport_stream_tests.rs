...
use gent_types::{
    ...
    DecisionSettlement, DoctorReport, Event, EventResume, HostEpoch, HostStatus, PROTOCOL_MAX,
...
#[derive(Clone, Debug)]
struct StreamRuntime(Arc<Mutex<Vec<Event>>>);
...
async fn negotiated_attach_replays_then_delivers_later_events_in_order() {
    ...
    let runtime = StreamRuntime(Arc::clone(&events));
...
async fn attach_is_rejected_without_the_negotiated_stream_capability() {
    let runtime = StreamRuntime(Arc::new(Mutex::new(Vec::new())));
    let (mut client, server) = duplex(16 * 1024);
    let task = tokio::spawn(serve_connection(server, runtime));
