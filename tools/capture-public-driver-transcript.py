...
def normalized_frames(vendor: str, scenario: str) -> list[dict[str, object]]:
    ...
    if scenario == "steer":
        return [
            {"in": {"nativeType": "Bash", "status": "started"}, "expect": "tool_started",
             "expectFields": {"tool": "Bash"}},
            {"in": {"nativeType": "user", "source": "stream_json", "afterToolUse": True,
                    "markerRedacted": True}, "expect": "steer_message_accepted",
             "expectFields": {"steered": True}},
            {"in": {"nativeType": "result", "terminal": True}, "expect": "completed_turn",
             "expectFields": {"terminal": True}},
        ]
...
def metadata(args: argparse.Namespace, binary: Path, transport: str,
    ...
    value: dict[str, object] = {
        ...
        "cliVersion": version(binary), "adapterSpecVersion": "1", "appVersion": "0.1.3",
